...
| `GET/POST /api/ai/compare` | Model comparison: `POST { prompt, models? }` runs the 2–6 ids you pick (or this deployment's default lanes) and holds one credit per lane, refunding any lane that cannot answer; `POST { action:"mix", prompt, lanes:[{id,reply}] }` folds a chosen subset of the answers into a fresh combined one without re-asking any model (one lane's price); `GET` returns the lane contract — `minLanes`/`maxLanes`, `perLane`, `mixCost`, and the `defaults` a run starts from |
